	...
	Ability:Begin(Caster, {
		...
		{0.27, function()
			if M1_Count == 5 and not IsDiveKick and not IsSlam then
				...
				--Caster:Walk(Ability:FromData('Walk_Time') + 0.3, -2)
			end
		end},
		...
		{0, Ability:FromData("Hit_Times", M1_Count), function()
		...
		{Ability:FromData("Hit_Times", M1_Count), function()
			...
			if IsSlam or IsDiveKick then return end
		...
		{.567, function()
			...
			if not (M1_Count == 4) or (IsSlam or IsDiveKick) then return end
		...
		-- // SLAM & DIVEKICK MOVEMENT OR HITBOX
		{0.2, function()
		...
		{.5, function()
			...
			if (M1_Count ~= 2 and not IsSlam) or IsDiveKick then return end
			local Track = if IsSlam then 'Characters.Naruto.Abilities.M1.Victim_3' else nil
			...
			Ability:CreateHitbox(Caster, Vector3.zAxis*-3, Vector3.one * 5, function(Target)
				Ability:Hit(Caster, Target, {EffectData = Effect_Data, NoHitStop = true, Track = Track})

				if IsSlam then
					Ability:Effect("GroundRocksTrail", Target, 0.5, false)
				end
			end)
		end,},
	})
