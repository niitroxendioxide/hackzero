...
return function(Enemy: Types.EnemyClass, Data: {})
	...
	local Connection; Connection = RunService.Heartbeat:Connect(function(delta)
		...
		local MaxVal = Enemy.__Status:GetStat('Max_Health')
		local TotalDaze = peek(Enemy.__Daze)
		local HealthValue = math.clamp(SpringVal / MaxVal, 0, 1)
		local DazeValue = math.clamp(TotalDaze / Enemy.__Status.__Max_Daze, 0, 1)
		...
		Gui.Meters.StunPercent.Text = math.floor(((TotalDaze or 0) / Enemy.__Status.__Max_Daze) * 100)
		Gui.Meters.StunPercentBG.Text = Gui.Meters.StunPercent.Text
		
		Gui.Meters.StunDamageBonus.Visible = IsDazed
		
		local Mult = math.clamp((TotalDaze / Enemy.__Status.__Max_Daze) / 0.25, 0, 1)
		if IsDazed then
		...
		-- >>
		local SpringVal = peek(Affliction_Spring)

		local LowPart = math.clamp(SpringVal / 50, 0, 1)
		local TopPart = math.clamp((SpringVal - 50) / 50, 0, 1)
