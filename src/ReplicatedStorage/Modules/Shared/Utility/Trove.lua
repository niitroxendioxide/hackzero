...
function Trove:RemoveObject(Object: any)
	for _, TroveObj in self._objects do
		if TroveObj[1] == Object then
			table.remove(self._objects, table.find(self._objects, TroveObj))
		end
	end
end
...
function Trove:_cleanupObject(object, cleanupMethod)
	...
	elseif cleanupMethod == THREAD_MARKER then
		if object == coroutine.running() then return end
